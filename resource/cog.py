...
class command(commands.Cog):
    ...
    async def on_voice_state_update(self,member,before,after):
        if after.channel!=before.channel:
            if before.channel!=None and len(before.channel.members)==0 and len(before.channel.changed_roles)==0:
            ...
            if after.channel!=None and len(after.channel.members)==1 and len(after.channel.changed_roles)==0:
                sendch=member.guild.system_channel
                unix=int(time.time())
                data={
